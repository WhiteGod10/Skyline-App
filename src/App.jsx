...
const computeKPIs = (sessions, settings) => {
  ...
  const monthlyRollup = (() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      result.push({
        key, label,
        paid: sum(sess, "paidRevenue"),
        pending: sum(sess, "pendingRevenue"),
        hours: sess.reduce((a, s) => a + (typeof s.hours === "number" ? s.hours : 0), 0),
        count: sess.length,
      });
    }
    return result;
...
function RevenueArcGauge({ weeklyPaid, weeklyGoal, weeklyPct, weeklyMet, weeklyGap, weekSessions, hasImport, weekStart }) {
  const W = 320, cy = 172, r = 144;
  const arcLen = Math.PI * r; // ~452

  // SVG arc: semicircle from left to right through top
  // counter-clockwise (sweep=0) on SVG coords = visually going over the top
  const lx = W / 2 - r, rx = W / 2 + r;
  const arcPath = `M ${lx} ${cy} A ${r} ${r} 0 0 0 ${rx} ${cy}`;

  const fillDash = `${arcLen * weeklyPct} ${arcLen + 10}`;

  // Color ramp: red → orange → gold → green
  const fillColor = weeklyMet ? "#30D158"
    : weeklyPct > 0.75 ? "#FFD60A"
    : weeklyPct > 0.5  ? "#FF9F0A"
    : weeklyPct > 0.25 ? "#FF6B35"
    : "#FF453A";

  // Tick dots at 25 / 50 / 75 %
  const tickPcts = [0.25, 0.5, 0.75];
  const tickPoints = tickPcts.map(t => {
    const ang = Math.PI * (1 - t); // π=left, 0=right
    return { x: W / 2 + r * Math.cos(ang), y: cy - r * Math.sin(ang) };
  });
  ...
  // Text Y positions inside arc
  const amtY  = cy - r * 0.44;  // big number
  const subY  = cy - r * 0.24;  // "of $X this week"
  ...
  return (
    <div style={{ marginTop: 32, paddingBottom: 8 }}>
      ...
      <div style={{
        ...
        letterSpacing: 1.8, color: "var(--muted)", textTransform: "uppercase",
        marginBottom: 0,
      }}>
        {weekLabel}
      </div>
      ...
      <div style={{ position: "relative", width: "100%", margin: "0 auto" }}>
        <svg viewBox={`0 0 ${W} 185`} style={{ width: "100%", display: "block", overflow: "visible" }}>

          {/* Soft ambient glow behind fill */}
          {weeklyPct > 0 && (
            <path d={arcPath} fill="none" stroke={fillColor} strokeWidth={44}
              strokeDasharray={fillDash} strokeLinecap="round"
              opacity={0.10}
          ...
          {/* Tick dots (sit on track, rendered before fill so fill covers them) */}
          {tickPoints.map((pt, i) => (
            <circle key={i} cx={pt.x} cy={pt.y} r={3}
              fill={weeklyPct > tickPcts[i] ? fillColor : "rgba(255,255,255,0.15)"}
              style={{ transition: "fill 0.4s ease" }}
            />
          ))}

          {/* Fill arc */}
          <path d={arcPath} fill="none" stroke={fillColor} strokeWidth={16}
            strokeDasharray={fillDash} strokeLinecap="round"
            style={{ transition: "stroke-dasharray 1s cubic-bezier(0.34,1.05,0.64,1), stroke 0.4s ease" }}
          />
          ...
          {/* Arc end labels: $0 left, goal right */}
          <text x={lx + 4} y={cy + 18} textAnchor="start"
            style={{ fontFamily: "sans-serif", fontSize: 10, fill: "rgba(255,255,255,0.18)", fontWeight: 600 }}>
            $0
          </text>
          <text x={rx - 4} y={cy + 18} textAnchor="end"
            style={{ fontFamily: "sans-serif", fontSize: 10, fill: "rgba(255,255,255,0.18)", fontWeight: 600 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: -4, marginBottom: 20 }}>
        <div style={{
          fontSize: 12, fontWeight: 700, padding: "5px 16px", borderRadius: 99,
          ...
          {weekSessions.map((s, i) => {
            ...
            <div key={i} style={{
              ...
              <div style={{
                ...
                background: s.status?.toLowerCase() === "paid" ? "var(--success)" : "var(--accent)",
              }} />
              ...
              <span style={{
                ...
                background: s.status?.toLowerCase() === "paid" ? "rgba(48,209,88,0.1)" : "rgba(255,159,10,0.1)",
                color: s.status?.toLowerCase() === "paid" ? "var(--success)" : "var(--accent)",
              }}>
                {s.status || "—"}
              </span>
              <span style={{ fontSize: 13, fontWeight: 600, color: s.paidRevenue > 0 ? "var(--text)" : "var(--muted)", flexShrink: 0 }}>
      ...
      {/* No data nudge */}
      {!hasImport && (
        <div style={{ textAlign: "center", padding: "0 40px 8px", fontSize: 12, color: "rgba(255,255,255,0.18)", lineHeight: 1.6 }}>
          Import your spreadsheet in Insights to see revenue here
        </div>
      )}
...
/* ================================================================
   INSIGHTS SCREEN
================================================================ */
function InsightsScreen({ leads, tasks, settings, onUpdateSettings, sessions, onImport, onClearSessions }) {
  ...
  const weeklyPaid = weekSessions.reduce((s, r) => s + r.paidRevenue, 0);
  const weeklyGap = Math.max(0, settings.weeklyRevenueGoal - weeklyPaid);
  const weeklyPct = Math.min(weeklyPaid / settings.weeklyRevenueGoal, 1);
  const weeklyHours = weekSessions.reduce((s, r) => s + (r.hours || 0), 0);
  ...
  const revenueProgress = Math.min(metrics.monthRevenue / settings.monthlyRevenueGoal, 1);

  const updateSetting = (key, val) => {
    onUpdateSettings({ ...settings, [key]: Number(val) || 0 });
  };
    ...
    <div className="screen">
      <div className="screen-header">
        <div>
          ...
          <div className="screen-date">This month</div>
        </div>
      </div>
      ...
      {/* Weekly Revenue from Import */}
      <WeeklyRevenueCard
        sessions={sessions} weekSessions={weekSessions}
        weeklyPaid={weeklyPaid} weeklyGap={weeklyGap}
        weeklyPct={weeklyPct} weeklyHours={weeklyHours}
        weekStart={weekStart} goal={settings.weeklyRevenueGoal}
        dateRange={sessionDateRange}
        onImport={onImport} onClear={onClearSessions}
        weeklyRevenueGoal={settings.weeklyRevenueGoal}
      />
      ...
      <div style={{ padding: "0 16px", marginBottom: 16 }}>
        <div style={{ background: "var(--surface)", borderRadius: "var(--radius)", padding: "16px", border: "1px solid var(--border)" }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.5px", marginBottom: 8 }}>${metrics.monthRevenue.toLocaleString()}</div>
